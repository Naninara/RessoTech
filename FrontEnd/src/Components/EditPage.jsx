...
function EditPage() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3500/user/getsingle/${id}`, {
  ...
  const handleSubmit = () => {
    if (!inputData.end || !inputData.start || !inputData.name) {
      setErrors(true);
      return;
    }
    ...
    axios
      .patch(
        "http://localhost:3500/user/update",
        { ...inputData },
        {
          auth: { username: username, password: password },
        }
  ...
  const handleDelete = () => {
    axios
      .delete(`http://localhost:3500/user/delete/${id}`, {
        auth: { username: username, password: password },
      })
  ...
  return (
    <div className="w-full h-screen flex items-center justify-center  fixed top-0 left-0 backdrop-brightness-50 ">
      {response ? (
        <div className="bg-white flex flex-col gap-5 items-center p-[8%] rounded-2xl">
          <p>{response}</p>
          <Link to={"/dashboard"}>
            <button className="bg-green-500 p-3 rounded-xl text-white hover:scale-105 duration-300">
              Go To Home
            </button>
          </Link>
        </div>
      ) : (
        <div className=" w-[300px] md:w-[670px] h-auto flex flex-col z-20 bg-white rounded-md ">
          ...
          <div className=" overflow-scroll">
            <div className="w-auto h-[290px] px-[24px] py-[16px] flex flex-col gap-[24px]">
              <div className="w-auto h-[68px] gap-[4px] ">
                ...
                <input
                  type={"text"}
                  className="h-[44px] px-[12px] py-[4px] w-[250px] md:w-[622px] border-[1px] border-[#B9C0CB] rounded-md"
                  placeholder="Text"
              ...
              <div className="flex flex-col md:flex-row gap-[10px] text-[12px]">
                <div className="flex flex-col gap-[4px]">
                  <h4>Start date</h4>
                  <input
                    type={"date"}
                    className="h-[44px] px-[12px] py-[4px] w-[250px] md:w-[306px] border-[1px] border-[#B9C0CB] rounded-md"
                    placeholder="Text"
                    onChange={(e) => {
                      setInputData((prev) => ({
                        ...prev,
                  ...
                  />
                  {errors && (
                    <p className="text-red-500">Please Fill Start Date</p>
                  )}
                </div>
                <div className="flex flex-col gap-[4px]">
                  <h4>End Date</h4>
                  ...
                  />
                  {errors && (
                    <p className="text-red-500">Please Fill End Date</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-[4px] text-[12px]">
                <h4>Status</h4>
                <div
                  className="h-[44px] px-[12px] py-[4px] w-[250px] md:w-[622px] border-[1px] border-[#B9C0CB] rounded-md flex justify-between items-center"
                  onClick={() => {
                ...
                <div className="h-[140px] flex flex-col absolute">
                  {dropDown && (
                    <>
                      <div
                        className="flex px-[16px] gap-[12px] h-[40px] bg-white cursor-pointer w-[250px] md:w-[620px] z-30 hover:bg-slate-300 rounded-sm items-center"
                        onClick={(e) => {
                          setInputData((prev) => ({
                            ...prev,
                            status: "In Progress",
          ...
          <div className="w-auto h-[52px] flex items-center px-[24px] justify-end gap-[10px] border-t-2 border-t-[#D8E0FD] ">
            <button
              className="h-[32px] w-[69px] px-[8px] py-[10px] text-[12px] bg-red-600 text-white flex items-center  justify-center  rounded-md"
